...
void test_http_client_e2e(void)
{
    ...
    SYN_Status status = syn_http_client_init(&client, "GET", host, port, "/api/status",
                                              NULL, NULL, 0, NULL, 0,
    ...
    int iterations = 0;
    while (client.state != SYN_HTTP_STATE_DONE && client.state != SYN_HTTP_STATE_ERROR && iterations < 100) {
    ...
    printf("[Integration Test] Nginx HTTP Response Code: %d, Body: %s\n",
           client.resp.status_code, rcv_body);
