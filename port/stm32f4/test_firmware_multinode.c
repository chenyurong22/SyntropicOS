...
extern SYN_Status syn_port_uart_transmit(SYN_UARTInstance inst, const uint8_t *data, size_t len,
                                         uint32_t timeout_ms);
...
int main(void)
{
    ...
    syn_cobs_decoder_init(&trans_ctx.cobs_dec, trans_ctx.cobs_rx_buf, sizeof(trans_ctx.cobs_rx_buf),
                          on_cobs_packet, &trans_ctx);

    SYN_Transport transport = {.send = uart_cobs_send, .recv = uart_cobs_recv, .ctx = &trans_ctx};
    ...
    while (1) {
        ...
        if (MY_NODE_ID == 1 && peer_found_flag && !message_sent) {
            ...
            if (now - last_send > 100) {
                ...
                bool sent = syn_router_send(&router, PEER_NODE_ID, 0x20, (const uint8_t *)msg,
                                            strlen(msg) + 1, true);
                if (sent) {
        ...
        if (now - last_stats >= 2000) {
            printf("Node %d: TX=%d, RX=%d, Drop=%d\n", MY_NODE_ID, (int)router.tx_count,
                   (int)router.rx_count, (int)router.drop_count);
            last_stats = now;
        }
