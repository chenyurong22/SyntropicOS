                         ...
                         const SYN_GPIO_Pin *digits, uint8_t num_digits, SYN_Seg7Type type)
{
    ...
    SYN_GPIO_State idle_seg_state = (type == SYN_SEG7_COMMON_CATHODE) ? SYN_GPIO_LOW : SYN_GPIO_HIGH;
    ...
    SYN_GPIO_State idle_digit_state = (type == SYN_SEG7_COMMON_CATHODE) ? SYN_GPIO_HIGH : SYN_GPIO_LOW;
...
{
    if (seg == NULL || seg->num_digits == 0) return;

    /* Turn OFF previous active digit selector */
    SYN_GPIO_State off_digit_state = (seg->type == SYN_SEG7_COMMON_CATHODE) ? SYN_GPIO_HIGH : SYN_GPIO_LOW;
    ...
    SYN_GPIO_State on_digit_state = (seg->type == SYN_SEG7_COMMON_CATHODE) ? SYN_GPIO_LOW : SYN_GPIO_HIGH;
...
{
    if (seg == NULL || digit_idx >= seg->num_digits) return;
