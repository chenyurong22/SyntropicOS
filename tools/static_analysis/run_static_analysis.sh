...
if command -v scan-build >/dev/null 2>&1; then
    mkdir -p build/static build/scan-build-out
    scan-build -o build/scan-build-out --status-bugs make -j "${JOBS}" -f tests/Makefile.unity test_unity BUILD_DIR=build/static || {
        echo "=== SCAN-BUILD BUG DETAILS ==="
        find build/scan-build-out -name "*.html" -exec grep -H -C 3 "BUGDESC" {} + || true
        find build/scan-build-out -name "*.html" -exec grep -H -C 3 "td class=\"DESC\"" {} + || true
        exit 1
    }
