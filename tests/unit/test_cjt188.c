...
void test_cjt188_edge_cases_and_nulls(void)
{
    ...
    TEST_ASSERT_EQUAL(0, syn_cjt188_encode_read_req(NULL, 32, 0x10, meter_id, vendor_id, 0x901F, 0));
    TEST_ASSERT_EQUAL(0, syn_cjt188_encode_read_req(buf, 10, 0x10, meter_id, vendor_id, 0x901F, 0));
    TEST_ASSERT_EQUAL(0, syn_cjt188_encode_valve_ctrl(NULL, 32, 0x10, meter_id, vendor_id, true, 0));
    ...
    /* Preamble only */
    uint8_t preamble_only[15] = {0xFE, 0xFE, 0xFE, 0xFE, 0xFE, 0xFE, 0xFE, 0xFE, 0xFE, 0xFE, 0xFE, 0xFE, 0xFE, 0xFE, 0xFE};
    ...
    uint8_t bad_start[] = {0x69, 0x10, 0x44, 0x33, 0x22, 0x11, 0x00, 0x33, 0x78, 0x01, 0x03, 0x1F, 0x90, 0x00, 0x80, 0x16};
    TEST_ASSERT_FALSE(syn_cjt188_parse_frame(bad_start, sizeof(bad_start), &frame));

    uint8_t bad_end[] = {0x68, 0x10, 0x44, 0x33, 0x22, 0x11, 0x00, 0x33, 0x78, 0x01, 0x03, 0x1F, 0x90, 0x00, 0x80, 0x17};
    TEST_ASSERT_FALSE(syn_cjt188_parse_frame(bad_end, sizeof(bad_end), &frame));

    uint8_t bad_cs[] = {0x68, 0x10, 0x44, 0x33, 0x22, 0x11, 0x00, 0x33, 0x78, 0x01, 0x03, 0x1F, 0x90, 0x00, 0x81, 0x16};
    TEST_ASSERT_FALSE(syn_cjt188_parse_frame(bad_cs, sizeof(bad_cs), &frame));

    /* Frame with data length < 3 (empty payload) */
    uint8_t short_data[] = {0x68, 0x10, 0x44, 0x33, 0x22, 0x11, 0x00, 0x33, 0x78, 0x01, 0x00, 0x00, 0x16};
    ...
    for (int i = 0; i < 9; i++) syn_cjt188_decoder_feed(&decoder, 0x00, &frame);
    ...
    uint8_t truncated_len[] = {0x68, 0x10, 0x44, 0x33, 0x22, 0x11, 0x00, 0x33, 0x78, 0x01, 0x14, 0x1F, 0x90, 0x00, 0x16};
