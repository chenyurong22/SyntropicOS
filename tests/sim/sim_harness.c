...
SYN_MotorOutput syn_dc_motor_output(SYN_DCMotor *motor)
{
    (void)motor;
    SYN_MotorOutput out = {
        .ctx = NULL, .set_output = mock_set_output, .coast = mock_coast, .brake = mock_coast};
    return out;
}
        ...
        .params =
            {
                ...
                .motor_Km = 600.0,
                .motor_Kb = 0.005,
                .friction_static = 40.0,
                .friction_coulomb = 25.0,
        ...
        .params =
            {
                ...
                .motor_Km = 600.0,
                .motor_Kb = 0.005,
                .friction_static = 100.0,
                .friction_coulomb = 60.0,
                .friction_viscous = 0.5,
                .driver_deadband = 5.0,
...
static void init_plant_and_ctrl(int index)
{
    ...
    SYN_MotorCtrl_Config cfg = {.motor = syn_dc_motor_output(&g_dc_motor),
...
static void run_full_autotune(int index, TestResult *res)
{
    ...
    SYN_AutoTune_Limits limits = {
        .position_min = (index == 8)
                            ? (int32_t)(sc->start_m * sc->params.counts_per_meter)
                            : (int32_t)(sc->params.track_min_m * sc->params.counts_per_meter),
        .position_max = (index == 8)
                            ? (int32_t)(sc->start_m * sc->params.counts_per_meter + 10)
                            : (int32_t)(sc->params.track_max_m * sc->params.counts_per_meter),
    ...
    if (index == 7)
        method = SYN_ATUNE_ZN_NO_OVERSHOOT;

    /* Coverage: Trigger Watchdog for index 11 */
    if (index == 11)
        limits.watchdog_ms = 1;
    ...
    if (index == 5) {
        ...
        if (index == 13)
            flags = SYN_ATUNE_FLAG_IDENT_KV; /* Skip PID to hit line 395 */
        ...
        SYN_AutoTune_Config cfg = {
            ...
            .test_output =
                (index == 8)
                    ? 20
                    : ((index >= 6 && index <= 7) ? 40 : ((index == 13 || index == 5) ? 50 : 0)),
    ...
    for (; steps < max_steps && at.state != SYN_ATUNE_DONE && at.state != SYN_ATUNE_ABORTED;
         steps++) {
...
int main(void)
{
    ...
    for (int i = 0; i < NUM_SCENARIOS; i++) {
        ...
        fprintf(stderr, "Scenario %d (%s): state=%d, reason=%d, gains=(%d,%d,%d), ff=(%d,%d)\n", i,
                g_scenarios[i].name, res.at_state, res.at_reason, res.kp, res.ki, res.kd, res.ff_kv,
                res.ff_ka);
    }
    return 0;
}
