...
static void fb_to_rgba(const uint8_t *fb, uint8_t *rgba, int w, int h, int scale)
{
...
static void render_frame(int32_t page, bool select, bool back, int32_t enc_delta, bool touch_down,
                         int16_t touch_x, int16_t touch_y)
{
    ...
    if (s_page == 0) {
        ...
        syn_imgui_layout_begin(&s_gui_ctx, 0, 0, 128);
        syn_imgui_text_marquee(&s_gui_ctx, "SyntropicOS Embedded UI Engine", &s_marquee_offset, 0,
                               0, 0, 1);
        if (syn_imgui_collapsing_header(&s_gui_ctx, "System Config", &s_collapse1_open, 0, 0, 0,
                                        0)) {
            syn_imgui_label(&s_gui_ctx, "Mode: Active", 0, 0);
            syn_imgui_label(&s_gui_ctx, "Volt: 3.3V", 0, 0);
        }
        if (syn_imgui_collapsing_header(&s_gui_ctx, "Network Status", &s_collapse2_open, 0, 0, 0,
                                        0)) {
...
static void parse_query(const char *query, int32_t *page, int32_t *enc, bool *sel, bool *back,
                        bool *touch, int16_t *tx, int16_t *ty)
{
    ...
    if (strstr(query, "select=1"))
        *sel = true;
    if (strstr(query, "back=1"))
        *back = true;
...
static void handle_connection(int fd)
{
    ...
    if (strcmp(path, "/fb") == 0) {
        ...
        send_response(fd, 200, "OK", "application/octet-stream", s_framebuf, FB_SIZE);
        ...
        if (f) {
            ...
            if (png && fread(png, 1, (size_t)sz, f) == (size_t)sz) {
                ...
                send_response(fd, 500, "Error", "text/plain", (const uint8_t *)"read error", 10);
            }
            free(png);
            fclose(f);
        }
        ...
        static const char *index_html =
            ...
            "</pre></body></html>";
        send_response(fd, 200, "OK", "text/html", (const uint8_t *)index_html, strlen(index_html));
    }
}
