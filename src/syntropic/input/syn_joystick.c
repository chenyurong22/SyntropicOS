...
void syn_joystick_feed_adc(SYN_Joystick *joy, uint16_t raw_x, uint16_t raw_y, bool btn)
{
    ...
    if (dx > -((int32_t)joy->deadband) && dx < (int32_t)joy->deadband) dx = 0;
    if (dy > -((int32_t)joy->deadband) && dy < (int32_t)joy->deadband) dy = 0;
    ...
    /* Clamp percentages */
    if (joy->current_x_pct > 100) joy->current_x_pct = 100;
    if (joy->current_x_pct < -100) joy->current_x_pct = -100;
    if (joy->current_y_pct > 100) joy->current_y_pct = 100;
    if (joy->current_y_pct < -100) joy->current_y_pct = -100;
...
{
    if (joy == NULL) return SYN_JOYSTICK_DIR_CENTER;
    return joy->dir;
}
