...
{
    if (sr == NULL || bit_index >= (uint16_t)(sr->num_chips * 8)) return;
...
{
    if (sr == NULL || chip_index >= sr->num_chips) return;
...
void syn_shiftreg_in_read(SYN_ShiftRegIn *sr)
{
    ...
    for (int c = (int)sr->num_chips - 1; c >= 0; c--) {
        ...
        for (int b = 0; b < 8; b++) {
            ...
            if (sr->msb_first) {
                if (bit) val |= (uint8_t)(0x80 >> b);
            } else {
                if (bit) val |= (uint8_t)(0x01 << b);
            }
...
{
    if (sr == NULL || bit_index >= (uint16_t)(sr->num_chips * 8)) return false;
...
{
    if (sr == NULL || chip_index >= sr->num_chips) return 0;
