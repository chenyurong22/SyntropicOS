...
SYN_Status syn_smbus_encode_packet(const SYN_SMBUS_Packet *pkt, uint8_t *tx_buf, size_t buf_size, size_t *out_len)
{
    ...
    case SYN_SMBUS_PROTO_WRITE_WORD:
        if (buf_size < 4 + (pkt->pec_enabled ? 1 : 0)) return SYN_ERROR;
        tx_buf[idx++] = addr_rw_write;
        tx_buf[idx++] = pkt->command;
    ...
    case SYN_SMBUS_PROTO_WRITE_BLOCK: {
        uint8_t payload_len = (pkt->length > SYN_SMBUS_BLOCK_MAX_LEN) ? SYN_SMBUS_BLOCK_MAX_LEN : pkt->length;
        if (buf_size < (size_t)(3 + payload_len + (pkt->pec_enabled ? 1 : 0))) return SYN_ERROR;
        tx_buf[idx++] = addr_rw_write;
        tx_buf[idx++] = pkt->command;
...
SYN_Status syn_smbus_decode_packet(SYN_SMBUS_Packet *pkt, const uint8_t *rx_buf, size_t rx_len, SYN_SMBUS_Protocol proto, bool has_pec)
{
    ...
    default:
        pkt->length = (uint8_t)(data_len > SYN_SMBUS_BUF_MAX_LEN ? SYN_SMBUS_BUF_MAX_LEN : data_len);
        if (pkt->length > 0) {
