...
static void fsm_on_start(void *ctx)
{
    (void)ctx;
    fsm_action_called = 1;
}
static void fsm_on_stop(void *ctx)
{
    (void)ctx;
    fsm_action_called = 2;
}

static bool fsm_guard_deny(void *ctx)
{
    (void)ctx;
    return false;
}
...
static void on_enter_run(void *ctx)
{
    (void)ctx;
    enter_run_count++;
}
static void on_exit_run(void *ctx)
{
    (void)ctx;
    exit_run_count++;
}

static bool guard_allow(void *ctx)
{
    (void)ctx;
    return true;
}
...
static const SYN_FSM_StateDesc test_fsm_descs[] = {{FSM_ST_IDLE, on_enter_idle, on_exit_idle},
                                                   {FSM_ST_RUNNING, on_enter_run, on_exit_run},
                                                   SYN_FSM_STATE_END};

static const char *const test_state_names[] = {"IDLE", "RUNNING", "ERROR"};
