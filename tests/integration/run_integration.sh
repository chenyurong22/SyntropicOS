...
CORE_PORT="src/port/posix/port_posix_socket.c tests/unit/unity/unity.c tests/unit/mocks/mock_port.c"

mkdir -p build/tests

echo "=== Building & Compiling Integration Test Drivers ==="
gcc ${CFLAGS} src/syntropic/net/syn_mqtt.c src/syntropic/util/syn_fmt.c src/syntropic/crypto/syn_hkdf.c src/syntropic/util/syn_sha256.c src/syntropic/util/syn_metrics.c src/syntropic/net/syn_router.c src/syntropic/system/syn_errlog.c ${CORE_PORT} tests/integration/test_mqtt_integration.c -o build/tests/test_mqtt_integration -lm &
