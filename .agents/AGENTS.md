...
- **Test Reporting & Error Honesty**: Read stdout/stderr completely **after the process exits**. ALWAYS explicitly report the exact test pass/fail metrics summary line (`X Tests Y Failures Z Ignored`) for both unit tests and bare-metal QEMU emulation runs. If any test fails or static analysis flags an issue, loudly report `FAILURE:` followed by exact error. Never hide or downplay failing tests. Never read partial output from a still-running process and conclude success.
